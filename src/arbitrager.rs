    ...
    RootProvider<T>: Provider + Clone,
{
    ...
    for target in &config.targets {
        ...
        // (dex, pool, price, spread_bps, fee_bps, net_profit_bps)
        let mut best_opportunity: Option<(String, String, f64, f64, f64, f64)> = None;
                    ...
                    Ok(pool_info) => {
                        ...
                        if net_profit_bps > 0.0 && spread_bps.abs() >= target.min_watch_spread_bps {
                            if best_opportunity.is_none() || net_profit_bps > best_opportunity.as_ref().unwrap().5 {
                                best_opportunity = Some(("Hyperswap".to_string(), pool_addr_str.clone(), price.token0_price, spread_bps, fee_bps, net_profit_bps));
                            }
                        }
                    }
                    Err(e) => {
            ...
            if let Ok(pool_addr) = Address::from_str(pool_addr_str) {
                match get_pool_info(provider.clone(), pool_addr).await {
                    Ok(pool_info) => {
                        ...
                        if net_profit_bps > 0.0 && spread_bps.abs() >= target.min_watch_spread_bps {
                            if best_opportunity.is_none() || net_profit_bps > best_opportunity.as_ref().unwrap().5 {
                                best_opportunity = Some(("ProjectX".to_string(), pool_addr_str.clone(), price.token0_price, spread_bps, fee_bps, net_profit_bps));
                            }
                        }
                    }
                    Err(e) => {
        ...
        if let Some((dex, pool, price, spread, fee, net_profit)) = best_opportunity {
            ...
            if net_profit >= target.min_executable_spread_bps.abs() {
                ...
                if target.statistical_arb {
                    // tracing::info!("  📈 Statistical arbitrage mode - would execute EVM-only trade");
                ...
                tracing::info!("  ⏸️  Net profit ({:.2} bps) below executable threshold ({} bps)", net_profit, target.min_executable_spread_bps.abs());
            }
        } else {
